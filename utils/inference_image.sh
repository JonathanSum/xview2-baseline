...
# Create the log/output directories if it doesn't exist 
...
python3 "$XBDIR"/utils/combine_jsons.py --polys "$label_temp"/"${input_image%.*}".json --classes /tmp/inference/classification_inference.json --output "$inference_base/inference_"${input_image%.*}".json" #>> "$LOGFILE" 2>&1

printf "\n" #>> "$LOGFILE"

# Transforming the inference json file to the image with the polygons and color coded damage on top

python3 "$XBDIR"/utils/overlay_predictions.py --image "$disaster_post_file" --json "$inference_base"/inference_"${input_image%.*}".json --output "$output_dir"/images/inference_"${disaster_post_file##*/}"

#Cleaning up by removing the temporary working directory we created
printf "Cleaning up\n"
mv "$inference_base"/inference_"${input_image%.*}".json "$output_dir"/jsons
